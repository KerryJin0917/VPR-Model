...
class Dinov2Encoder(nn.Module):
    def __init__(self, device="cuda"):
        super().__init__()
        self.device = device
        # Load the Small version (vits14) for efficiency (384-dim embeddings)
        # You can use 'dinov2_vitb14' for Base or 'dinov2_vitl14' for Large later
    ...
    def encode(self, images):
        ...
        # DINOv2 expects 224x224, matching your ImageDataset
        if images.shape[-1] != 224 or images.shape[-2] != 224:
            images = F.interpolate(images, size=(224, 224), mode='bilinear', align_corners=False)

        # DINOv2 returns the [CLS] token (global descriptor) by default
        features = self.backbone(images)

        # Normalize to unit length for cosine similarity (important for VPR)
